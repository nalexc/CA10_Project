...
A rotation of $\Phi$ around the unit vector $\vec{q_{1:3}}$ can be described according to Euler's formula.
\begin{equation}
...
\begin{equation}
\underline{R}(\vec{^s_i q(t)}) = (q_4^2 - \vec{q_{1:3}}^T \vec{q_{1:3}}) \underline{1} + 2 \vec{q_{1:3}} \vec{q_{1:3}}^T - 2q_4 \underline{q}_{1:3}^\times 
\end{equation}
%\begin{equation}
%\underline{R}(\vec{^s_i q(t)}) = \underline{ 1} + 2  \underline{q}_{1:3} ^\times + 2 \underline{S^2} ^\times (\epsilon) 
%\end{equation}

\subsection{Quaternion multiplication}
Let \textbf{q} represent the unit rotation quaternion, with \textbf{i}, \textbf{j}, \textbf{k} being the base vectors in Euclidean space and $q_4$ as the scalar part: 
\begin{equation}
...
\begin{flalign}   
	...
	\label{eq:sfgdm}
\end{flalign}
A skew-symmetric matrix is given as $\underline q^\times$ and defined as
\begin{flalign}   
	\underline q^\times = 
	\begin{bmatrix}
		 0 & - q_{A_{3}}  & q_{A_{2}}\\
...
\begin{flalign}   
	...
	\begin{bmatrix}
		 - \underline  q^\times + \underline{\vec 1} q_{C_{4}} & \vec{ q} \\
