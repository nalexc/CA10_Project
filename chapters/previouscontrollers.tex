...
\end{flalign}
with $F = \alpha\ast diag[111]$ be a positive definite matrix. For $s=0$, $\vec{\tilde{\omega}} = - \alpha\ast\vec{\tilde{q}_{1:3}}$ and thus $\alpha$ can be designed appropriately to give the desired convergence, more details can be found in \todo{create appendix}   