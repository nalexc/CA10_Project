...
\end{flalign}
%  
and the output vector can be written as
\begin{flalign}
\vec{y} = \underline{C}\vec{\omega}
\label{eq:seom2554}
\end{flalign}
where $\underline{A}=-\underline{I}_{s}^{-1}  [(\underline{I}_{s}\vec{\omega})^\times- (\vec{h_{rw}})^\times] $ is the system matrix, $ \underline{B}= \underline{I}_{s}^{-1}$ is the input matrix and $\vec{u} =\vec{N_{rw}} + \vec{N_{mt}} $ is the input vector and $\vec{N_{dist}}= \vec{d}$ is the disturbance vector. The system can now be written in Luenberger-like form as
%


%\begin{flalign}
%\^{\vec{\dot \omega}} = \underline{A}{\^{\vec{\omega}}}+\underline{B}\vec{u}+\underline{B}\vec{d}+\underline{L}\underline{C}({\vec{\omega}}-{\^{\vec{\omega}}})
%\label{eq:seom255554}
%\end{flalign}

% 
with $\underline{C}$ be identity matrix. From \eqref{eq:seom2244} it can be seen 