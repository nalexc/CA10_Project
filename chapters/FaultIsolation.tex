...
\subsection{Magnetorquer Reconfiguration}
...
\begin{figure}[ht]  
	...
	\end{minipage}
	\caption{To the left are depicted the magnetorquers of each axis $x,y,z$ respectively and to the right their pairs}
	\label{fig:magneticconfig}
\end{figure}  
%
Even the combination between the two methods gives a desired result in the sense of isolation, the structural analysis method cant detect all kind of faults compared to the Luenberger-like Observer which can detect the majority of faulty situations. In this case the $AND$ operation can be changed with $OR$ operation and in the presence of a fault which structural analysis is not able to detect, all the magnetorquers will swapped with their pairs.  
