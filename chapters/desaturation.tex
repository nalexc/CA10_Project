...
\begin{equation}
\underline{J}\vec{\omega} + \underline{\omega}^\times\underline{J}\vec{\dot{\omega}} = \vec{u}
\end{equation}

\begin{equation}
\vec{\tau_m}^{[I]} = -\frac{\underline{\tilde{b}}^\times(t)}{|\vec{\tilde{b}}(t) |^2} k_p\left(\vec{h_w}^{[I]} - \underline{R}^T(\vec{q})\vec{h_{ref}} \right)
\end{equation}
