...
\end{flalign}
with $\underline{F} = \alpha\ast diag[111]$ be a positive definite matrix. It can be seen that for $s=0$, the motion on the sliding surface is governed by $\vec{\tilde{\omega}} = - \alpha\ast\vec{\tilde{q}_{1:3}}$, where $\vec{\tilde{q}_{1:3}}$ denotes the vector part of the quaternion, and $\alpha$ is chosen appropriately through trial and error to give the desired convergence for $\vec{\tilde{q}}$ which for the desired alignment between frames is $[0;0;0;1]$. In order to prove this, differentiation of \eqref{eq:smallsignal22} following \cite{TH} is written as
%
\begin{flalign}
...
%
\begin{flalign}
\dot{\tilde{q_{4}}}  = -\frac{1}{2} \dot{\tilde{\omega}} 
